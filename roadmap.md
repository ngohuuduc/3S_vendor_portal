...
- **Portal → Odoo (DO):** **XML-RPC write real-time qua PATCH** — vendor ghi SL Giao lên `stock.move.product_uom_qty` ngay khi bấm Lưu. **KHÔNG có 23:00 cutoff job push.** 23:00 chỉ là UI lock trên Portal
...
4. Mỗi lần bấm Lưu, portal ghi xuống Odoo **real-time qua PATCH**: `stock.move.product_uom_qty`. Không có batch push tại 23:00.
...
- `stock.move` → product_id, product_uom_qty (SL Giao do vendor ghi), quantity_done (SL Thực Nhận do cửa hàng chốt)
...
| GET | `/api/return-orders/{id}` | Chi tiết RPO với các dòng trả hàng và Return Order liên kết |
| GET | `/api/return-notes/{rn_id}` | Chi tiết Return Order với các dòng sản phẩm và ngày lấy hàng |
| PATCH | `/api/return-notes/{rn_id}/pickup-date` | Đặt hoặc cập nhật ngày lấy hàng trên RO (bị chặn nếu đã ký) — tên endpoint giữ `return-notes` để tránh API breaking |
| POST | `/api/return-notes/{rn_id}/confirm-and-sign` | Xác nhận RPO + gửi PNG chữ ký trong một bước. Khoá RO, tạo PDF |
| GET | `/api/return-notes/{rn_id}/pdf` | Tải xuống RO PDF đã ký (cùng định dạng với DO PDF) |
...
- **Mới (New, trước cutoff):** `PATCH /api/delivery-orders/{do_id}/lines` ghi SL Giao real-time qua XML-RPC PATCH lên `stock.move.product_uom_qty`. **Không có endpoint sign** (ký điện tử đã bỏ).
...
**Không gửi email:** Vendor xác nhận PO (dữ liệu được đẩy lên Odoo theo thời gian thực), Vendor ký DO/RO (dữ liệu được đẩy lên Odoo tự động).
...
3. **Portal không gọi `button_validate`** — vendor chỉnh sửa DO; Odoo validation do cửa hàng thực hiện. Odoo picking giữ nguyên trạng thái `assigned` sau khi vendor lưu (qty real-time qua PATCH lên `product_uom_qty`)
