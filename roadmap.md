...
5. Portal gửi email đến nhà cung cấp xác nhận biên nhận **đơn thuần** (không kèm cảnh báo chênh lệch). Vendor xem **Log Note** trên DO Form để track việc cửa hàng có sửa `qty_done` hay không (mail.tracking.value)
...
4. Portal gửi email đến vendor xác nhận biên nhận **đơn thuần** (số PO, mã biên nhận, ngày confirm). **Không kèm cảnh báo chênh lệch.** Vendor xem **Log Note** trên DO Form để track việc cửa hàng có sửa `qty_done` hay không — Portal dùng `mail.tracking.value` để hiển thị lịch sử thay đổi (cả của vendor lẫn cửa hàng)
...
| Cửa hàng xác nhận biên nhận | Nhà cung cấp | Xác nhận biên nhận đơn thuần (số PO, mã biên nhận). **Không cảnh báo chênh lệch** — vendor xem Log Note nếu cần |
...
| `/return-orders/:id` | Chi tiết RPO | Nhà cung cấp | Các dòng trả hàng + Return Order liên kết |
| `/return-notes/:id` | Chi tiết RO | Nhà cung cấp | Các dòng sản phẩm trả hàng + bộ chọn ngày lấy hàng (qty chỉ đọc) + Xác nhận & Ký |
| `/return-notes/:id/pdf` | RO PDF | Nhà cung cấp | Tải xuống/in RO PDF đã ký |
