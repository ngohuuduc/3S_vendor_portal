...
| ~~Capture chữ ký~~ | **Đã bỏ** — không có ký điện tử trên Portal, vendor chỉ in PDF |
| Audit logging (DB) | Các hành động được ghi: `login`, `po_confirm`, `po_reject`, `po_auto_cancel`, `do_update`, `do_print`, `rn_print`, `receipt_validated`. **Đã bỏ `do_sign` (không có ký điện tử), `do_unlock` (không có admin unlock trên Portal — xem DO-LIVE-001 Blocker 6)** |
...
| 7 | **23:00 Cutoff** | Lúc 23:00 đêm trước ngày giao, DO chuyển sang **Đã Gửi (Sent)** trên Portal | ✅ **UI lock only** (derive từ `scheduled_date.date() <= today` UTC+7) — **KHÔNG có job push** (qty đã ghi real-time qua PATCH). Xem [DO-LIVE-001](IssueLog.md) Blocker 1 |
| 8 | **Receipt validation (Odoo → Portal)** | Cửa hàng xác nhận Receipt → Portal hiển thị DO state = Đã Hoàn Thành | ✅ **Real-time live query** từ `stock.picking.state` — **KHÔNG có nightly sync**. Xem [DO-LIVE-001](IssueLog.md) Blocker 1+6 |
| 9 | **Vendor deactivation** | Vendor deactivated trên Odoo → sync set `is_active = FALSE` | ✅ Chờ sync cycle 6h — không critical |

**Cơ chế sync (confirmed, đã update theo DO-LIVE-001):**
- **Portal → Odoo (PO):** XML-RPC write real-time — xác nhận/từ chối PO
- **Portal → Odoo (DO):** **XML-RPC write real-time qua PATCH** — vendor ghi SL Giao lên `stock.move.line.qty_done` ngay khi bấm Lưu (1 field duy nhất, sau đó cửa hàng có thể ghi đè khi validate). **KHÔNG có 23:00 cutoff job push.** 23:00 chỉ là UI lock trên Portal
- **Odoo → Portal (hồ sơ):** Scheduled batch job mỗi 6h
- **Odoo → Portal (dữ liệu PO):** XML-RPC on-demand, có cache, refresh mỗi 10 phút
- **Odoo → Portal (DO + Receipt validated):** **Live query real-time** — DO state derive từ `stock.picking.state`. **KHÔNG có nightly batch sync.**
...
### Luồng Delivery Order (DO) — đã update theo [DO-LIVE-001](IssueLog.md)
1. Khi nhà cung cấp xác nhận PO, Odoo tự động tạo 1 DO (`stock.picking`). Portal **live query** qua XML-RPC, pre-fill SL Giao = SL Đặt từ PO. **Không lưu local DB** (bảng `delivery_orders` + `delivery_order_lines` đã bị xoá theo DO-LIVE-001 Blocker 6).
2. Nhà cung cấp chỉnh sửa DO: chỉnh SL Giao (chỉ giảm, không tăng vượt SL đặt). Ngày giao cố định từ PO, không sửa được trên DO.
3. Nhà cung cấp có thể **lưu nhiều lần, in nhiều phiên bản** — tất cả khi DO ở trạng thái Mới (New). Không cần ký điện tử (đã bỏ).
4. Mỗi lần bấm Lưu, portal ghi xuống Odoo **real-time qua PATCH**: `stock.move.line.qty_done` (1 field duy nhất). Không có batch push tại 23:00.
5. Mỗi lần in, PDF được tạo on-the-fly (WeasyPrint) với dữ liệu mới nhất — vendor luôn có phiên bản cập nhật. Không lưu PDF trên server.
6. **Lúc 23:00 đêm trước ngày giao hàng:** DO chuyển sang **Đã Gửi (Sent)** trên Portal — **UI lock thuần tuý**, derive từ `scheduled_date.date() <= today (UTC+7)`. Không có job, không có push.
7. Cửa hàng nhập `quantity_done` trên Odoo trong ngày giao hàng. Khi cửa hàng confirm picking, DO trên Portal hiển thị **Đã Hoàn Thành (Done)** — real-time live query.
8. **Admin unlock:** không có endpoint trên Portal. Admin cập nhật `scheduled_date` trên Odoo (đẩy về tương lai) → lock condition tự giải phóng → vendor chỉnh lại trên Portal.
...
2. Cửa hàng xem xét Receipt trong Odoo — `qty_done` đã được Portal ghi real-time từ trước. Cửa hàng có thể chỉnh lại nếu hàng hư hỏng (ghi đè cùng field)
3. Cửa hàng xác nhận Receipt trong Odoo — `qty_done` được finalize, stock move được tạo
4. **Portal live query Odoo real-time** — phát hiện `stock.picking.state = done`, hiển thị DO state = **Đã Hoàn Thành (Done)**. Không có nightly sync, không có batch.
5. **Odoo gửi email trực tiếp** đến nhà cung cấp xác nhận biên nhận (đơn thuần, không cảnh báo chênh lệch). Portal **không** can thiệp vào việc gửi email này. Mỗi lần re-confirm picking, Odoo gửi lại. Vendor xem **Log Note** trên DO Form để track `qty_done` (mail.tracking.value)
6. Nhà cung cấp có thể thấy cả 2 cột SL Giao và SL Thực Nhận song song trên trang chi tiết DO (state Đã Hoàn Thành)
7. Nhà cung cấp có thể xuất dữ liệu dưới dạng PDF hoặc CSV để lập hoá đơn và đối soát
8. **Re-open trên Odoo:** nếu cửa hàng reopen picking để sửa `qty_done` sau khi đã validate, Portal live query tự cập nhật giá trị mới — DO trên Portal có thể chuyển ngược từ Đã Hoàn Thành → Đã Gửi (khi picking state về `assigned`) rồi → Đã Hoàn Thành lại khi store re-validate

### Cơ chế sync (Odoo → Portal) — đã update theo [DO-LIVE-001](IssueLog.md)
- **Live query real-time** qua XML-RPC — Portal đọc `stock.picking.state` trực tiếp mỗi khi render UI, không cần batch sync
- DO state derive từ Odoo: `assigned` (chưa qua cutoff) → Mới; sau cutoff `scheduled_date.date() <= today` → Đã Gửi; `done` → Đã Hoàn Thành
- Không cần Odoo module, chỉ cần quyền XML-RPC read trên `stock.picking`
- Job 23:00 còn lại: PO auto-cancel sau 7 ngày dương lịch. **Job cutoff DO + nightly sync receipt đã bị xoá** (DO-LIVE-001 Blocker 1+6)
- Portal → Odoo communication (xác nhận/từ chối PO, ghi SL Giao trên DO) là real-time via XML-RPC PATCH
...
- ~~`INTERNAL_NOTIFICATION_EMAIL`~~ — đã bỏ: thông báo cửa hàng gửi đến **Buyer** (`purchase.order.user_id`, fallback `create_uid`) + **Kho Nhận Hàng** (`picking_type_id.warehouse_id`) trên Odoo, không phải hộp thư chung
...
- **Odoo → Portal sync — đã giải quyết bởi [DO-LIVE-001](IssueLog.md):** Portal **live query** `stock.picking.state` qua XML-RPC tại thời điểm render UI — DO state derive real-time, không cần webhook/polling/batch. Đọc `qty_done` qua XML-RPC khi NCC mở chi tiết DO.
...
### Mục tiêu
- Định nghĩa các bảng DB: `vendor_users`, `admin_users`, `password_reset_tokens`, `return_notes`, `return_note_lines`, và `audit_log`. ⚠️ **DO-LIVE-001 đã xoá `delivery_orders` + `delivery_order_lines`** (migration `d0e1f2a3b4c5`) — DO giờ live query trực tiếp từ Odoo, không lưu local DB.
...
**~~`delivery_orders` + `delivery_order_lines`~~ — ĐÃ XOÁ**

Theo [DO-LIVE-001](IssueLog.md) (closed 2026-04-11), 2 bảng này đã bị xoá khỏi schema (migration `d0e1f2a3b4c5`). DO giờ được **live query trực tiếp từ Odoo** qua XML-RPC, không lưu local DB. Lý do: tránh nhập nhằng giữa local snapshot và state thực tế trên Odoo, đơn giản hoá cơ chế lock (chỉ derive từ `scheduled_date`).

**Dữ liệu DO đọc từ Odoo (không lưu local):**
- `stock.picking` → state, scheduled_date, partner_id, picking_type_id
- `stock.move` → product_id, quantity_done (1 field duy nhất: vendor ghi trong state Mới → cửa hàng có thể ghi đè khi confirm). Label hiển thị "SL Giao" hoặc "SL Thực Nhận" tuỳ state.
- `purchase.order.line` → product_qty (SL Đặt), price_unit, discount, taxes_id

**Helper functions** (`VendorScopedOdooClient`): `list_delivery_orders()`, `get_delivery_order()`, `write_picking_data()`, `_derive_do_status()`, `_is_do_locked()`, `_locked_at_str()`.
...
| Cột | Kiểu | Ghi chú |
|---|---|---|
| `id` | serial PK | ID RO nội bộ của portal |
...
| `vendor_id` | FK → vendor_users | vendor sở hữu RO này |
| `pickup_date` | date | ngày vendor đặt để lấy hàng trả về |
| `status` | varchar | `assigned` (Chờ Thu Hồi / Waiting) hoặc `done` (Đã Hoàn Thành / Done). Portal derive `done` khi Odoo `stock.picking.state = done`, bất kể do cửa hàng confirm tay hay Odoo scheduled action tự confirm sau hạn chót (`pickup_date` + 7 ngày dương lịch) |
| `created_at` | timestamp | |
| `updated_at` | timestamp | |

> **Đã bỏ:** `latest_signature` (không có ký điện tử), `locked_at` (không có cơ chế 23:00 cutoff cho RO — lock derive thuần từ `pickup_date + 7`)

**`return_note_lines`**
| Cột | Kiểu | Ghi chú |
|---|---|---|
| `id` | serial PK | |
| `rn_id` | FK → return_notes | RO cha (tên field giữ legacy `rn_id` để tránh breaking schema; tham chiếu `return_notes` table — schema rename là implementation concern) |
...
| `action` | varchar | một trong: `login`, `po_confirm`, `po_reject`, `po_auto_cancel`, `do_update`, `do_print`, `rn_print`, `receipt_validated`. **Đã bỏ:** `do_sign` (không có ký điện tử), `do_unlock` (không có admin unlock trên Portal) |
...
- Triển khai pipeline tạo PDF on-the-fly (WeasyPrint, không ký điện tử)
...
| POST | `/api/purchase-orders/{id}/reject` | Từ chối Sent RFQ — gọi `button_cancel` trên Odoo + email Buyer (`user_id`, fallback `create_uid`) + Kho Nhận Hàng (`warehouse_id`); trả về 409 nếu PO không ở trạng thái `sent` |
| GET | `/api/delivery-orders/{do_id}` | Chi tiết DO với các dòng sản phẩm, ngày giao, SL = `stock.move.quantity_done` (label đổi theo state: "SL Giao" khi Mới/Đã Gửi, "SL Thực Nhận" khi Đã Hoàn Thành) — **live query Odoo, không lưu local DB** |
| PATCH | `/api/delivery-orders/{do_id}/lines` | Cập nhật SL Giao trên DO — ghi real-time qua XML-RPC PATCH lên `stock.move.line.qty_done`. Trả 423 nếu `scheduled_date.date() <= today (UTC+7)` (Đã Gửi). Qty phải ≤ qty đặt (chặn cứng). |
| ~~POST `/api/delivery-orders/{do_id}/sign`~~ | — | **Endpoint đã bị xoá** — không có ký điện tử (vendor chỉ in PDF) |
...
| GET | `/api/return-orders/{id}` | Chi tiết RPO với các dòng trả hàng và Return Order (RO) liên kết |
| GET | `/api/return-notes/{rn_id}` | Chi tiết Return Order với các dòng sản phẩm và ngày lấy hàng — tên endpoint giữ `return-notes` để tránh API breaking |
| PATCH | `/api/return-notes/{rn_id}/pickup-date` | Đặt hoặc cập nhật `pickup_date` trên RO. Trả 423 nếu quá hạn chót (pickup_date + 7 ngày dương lịch). |
| GET | `/api/return-notes/{rn_id}/pdf` | Tải xuống RO PDF (cùng định dạng với DO PDF) — on-the-fly, không cần ký |
| GET | `/api/export` | Xuất dữ liệu dưới dạng PDF hoặc CSV. Query params: `format` (pdf/csv), `type` (individual/summary), `ids` (bulk), `date_from`, `date_to`. Bao gồm PO, DO, RPO, RO |
...
| Method | Endpoint | Mô tả |
|---|---|---|
| GET | `/api/admin/vendors` | Liệt kê tất cả tài khoản vendor với trạng thái, lần đăng nhập cuối, số DO Đã Hoàn Thành, số DO Mới/Đã Gửi đang chờ |
...
| GET | `/api/admin/delivery-orders/{do_id}/pdf` | Tải xuống DO PDF của bất kỳ vendor nào (on-the-fly, không ký) |
| ~~POST `/api/admin/delivery-orders/{do_id}/unlock`~~ | — | **Endpoint đã bị xoá** (DO-LIVE-001 Blocker 6) — admin mở khoá bằng cách cập nhật `scheduled_date` trên Odoo |
...
### Cách tiếp cận khoá DO — 23:00 Cutoff (đã update theo [DO-LIVE-001](IssueLog.md))
- **Mới (New, trước cutoff):** `PATCH /api/delivery-orders/{do_id}/lines` ghi SL Giao real-time qua XML-RPC PATCH lên `stock.move.line.qty_done` (1 field duy nhất). **Không có endpoint sign** (ký điện tử đã bỏ).
- **23:00 Cutoff:** **Không có scheduled job.** UI lock thuần tuý — endpoint `PATCH /lines` kiểm tra điều kiện `scheduled_date.date() <= today (UTC+7)` tại thời điểm request và trả về HTTP 423 nếu đúng.
- **Đã Gửi (Sent, sau cutoff):** `PATCH /lines` trả về HTTP 423. Vendor chỉ có thể in PDF phiên bản cuối. Status derive thuần từ `scheduled_date` — không có column DB.
- **Mở khoá:** **Không có endpoint trên Portal.** Admin reset `scheduled_date` trên Odoo (đẩy về tương lai) → lock condition tự giải phóng → vendor thấy DO trở về Mới ở request kế tiếp.
...
1. Chỉnh sửa DO: SL Giao (≤ qty đặt, chỉ giảm, lưu/in nhiều lần) — ngày giao cố định từ PO, không sửa được trên DO
2. 23:00 cutoff: DO chuyển sang Đã Gửi trên Portal (**UI lock only**) — qty đã được ghi real-time qua PATCH suốt state Mới, không có batch push tại 23:00

Odoo picking giữ nguyên trạng thái `assigned` sau cutoff — `qty_done` đã được vendor ghi real-time. Cửa hàng có thể chỉnh lại `qty_done` (ghi đè cùng field) trước khi confirm Receipt trên Odoo. Khi confirm xong, `stock.picking.state = done`.

### Receipt validated (Odoo → Portal) — Live query real-time
Cơ chế: Portal **live query** `stock.picking.state` qua XML-RPC mỗi lần render UI — không có nightly batch sync (xem [DO-LIVE-001](IssueLog.md)).

1. Cửa hàng confirm Receipt trong Odoo → `qty_done` finalized, `stock.picking.state = done`
2. Lần kế tiếp vendor mở DO trên Portal hoặc Portal refresh list view → live query phát hiện state = `done` → DO chuyển sang **Đã Hoàn Thành (Done)**
3. Portal đọc `qty_done` qua XML-RPC để hiển thị cột SL Thực Nhận song song với SL Giao
4. **Odoo gửi email trực tiếp** đến vendor xác nhận biên nhận (Portal **không** xử lý gửi email này). Đơn thuần, không cảnh báo chênh lệch. Mỗi lần re-confirm picking, Odoo gửi lại email. Vendor xem **Log Note** trên DO Form để track `qty_done` (Portal đọc `mail.tracking.value` để hiển thị lịch sử thay đổi của cả vendor lẫn cửa hàng)
5. **Re-open scenario:** nếu cửa hàng reopen picking để sửa `qty_done`, Portal live query tự cập nhật giá trị mới (DO trên Portal có thể tạm về Đã Gửi rồi về Đã Hoàn Thành sau khi store re-validate)
6. Vendor có thể đăng nhập portal để xem chi tiết và xuất PDF/CSV
...
| ~~`do_sign`~~ | ~~`POST /delivery-orders/:id/sign`~~ — **endpoint đã bị bỏ** (không có ký điện tử trên Portal — vendor chỉ in PDF) | — |
| ~~`do_unlock`~~ | ~~`POST /admin/delivery-orders/:id/unlock`~~ — **endpoint đã bị xoá** (DO-LIVE-001 Blocker 6). Admin mở khoá bằng cách cập nhật `scheduled_date` trên Odoo. | — |
...
| Vendor từ chối RFQ | **Buyer** (`purchase.order.user_id`, fallback `create_uid`) **+ Kho Nhận Hàng** (`picking_type_id.warehouse_id`) | PO bị từ chối + hủy trong Odoo; Portal gửi qua AWS SES |
| PO tự động hủy (7 ngày dương lịch) | Nhà cung cấp + **Buyer + Kho Nhận Hàng** | Portal trigger auto-cancel qua cron → Portal tự gửi email (xem [AUTOCANCEL-CRON-TIMING-001](IssueLog.md)) |
| Cửa hàng xác nhận biên nhận | Nhà cung cấp | **Email do Odoo gửi trực tiếp** (Portal không xử lý). Gửi mỗi lần confirm/re-confirm picking. Đơn thuần, không cảnh báo chênh lệch — vendor xem Log Note nếu cần |
| ~~DO được admin mở khoá~~ | — | **Không gửi email** — không có chức năng admin unlock trên Portal (DO-LIVE-001 Blocker 6). Admin reset `scheduled_date` trên Odoo trực tiếp, vendor sẽ thấy DO trở về state Mới ở lần truy cập kế tiếp |
| RPO được cửa hàng tạo | Nhà cung cấp | Gửi bởi Odoo nội bộ (nút Send by Email) — không qua AWS SES |

**Không gửi email:** Vendor xác nhận PO (dữ liệu được đẩy lên Odoo theo thời gian thực), Vendor chỉnh SL Giao trên DO/RO (dữ liệu được ghi lên Odoo real-time qua PATCH; không có ký điện tử trên Portal).

Tất cả email do portal gửi dùng AWS SES theo `preferred_language` của vendor cho email gửi đến vendor, và tiếng Việt cho email gửi đến cửa hàng. Email cửa hàng gửi đến **Buyer** (`purchase.order.user_id`, fallback `create_uid`) **+ Kho Nhận Hàng** (`picking_type_id.warehouse_id`) trên Odoo, không phải hộp thư chung. Email RPO + email confirm Receipt do **Odoo gửi trực tiếp** (Portal không xử lý 2 loại email này).

### Cách tiếp cận pipeline DO / RO PDF
...
5. **Không nhúng chữ ký điện tử** — PDF có 2 ô trống cho chữ ký tay (vendor + cửa hàng) để ký trên giấy khi giao hàng

**Lưu ý:** RO PDF dùng **cùng layout và nội dung** với DO PDF, với tiêu đề đổi thành "Biên Bản Trả Hàng" và ngày giao hàng được thay thế bằng ngày lấy hàng.
...
**Footer:**
- Ghi chú vendor (header-level, nếu có) — `stock.picking.note`
- 2 ô chữ ký trống cho **chữ ký tay** trên giấy (vendor + cửa hàng) — ký vật lý khi giao hàng
- Timestamp in PDF (thời điểm vendor in)

> **Đã bỏ:** chữ ký điện tử PNG nhúng vào PDF — Portal không có chức năng ký điện tử (xem mục "Ký DO" trong Decisions). Vendor và cửa hàng ký bằng tay trên 2 bản giấy in ra.
...
- Triển khai form chỉnh sửa DO với validation số lượng (qty ≤ qty đặt, chặn cứng không tăng) và xử lý trạng thái khoá (UI lock when `scheduled_date.date() <= today`)
...
| `/delivery-orders/:id` | Chi tiết DO | Nhà cung cấp | Các dòng sản phẩm với SL Giao có thể chỉnh sửa (state Mới); ngày giao cố định từ PO; UI lock khi `scheduled_date.date() <= today` (state Đã Gửi) |
| ~~`/delivery-orders/:id/sign`~~ | — | — | **Route đã bị xoá** — không có ký điện tử |
| `/delivery-orders/:id/pdf` | DO PDF | Nhà cung cấp | Tải xuống/in DO PDF on-the-fly (không ký, có thể in nhiều lần) |
| `/returns` | Danh sách Trả Hàng | Nhà cung cấp | Danh sách RPO với tìm kiếm/lọc, badge trạng thái |
| `/return-orders/:id` | Chi tiết RPO | Nhà cung cấp | Các dòng trả hàng + Return Order (RO) liên kết |
| `/return-notes/:id` | Chi tiết RO | Nhà cung cấp | Các dòng sản phẩm trả hàng + bộ chọn `pickup_date` (qty chỉ đọc). **Không** có bước Xác nhận & Ký |
| `/return-notes/:id/pdf` | RO PDF | Nhà cung cấp | Tải xuống/in RO PDF on-the-fly (không ký) |
| `/profile` | Hồ sơ | Nhà cung cấp | Hồ sơ vendor chỉ đọc + tuỳ chọn ngôn ngữ |
| `/admin/dashboard` | Dashboard Admin | Admin | Thống kê tổng hợp: số lượng vendor, DO/RO đang chờ, trạng thái sync |
| `/admin/vendors` | Danh sách Vendor | Admin | Tất cả tài khoản vendor với trạng thái, lần đăng nhập cuối, số DO/RO |
| `/admin/vendors/:id` | Chi tiết Vendor | Admin | PO, DO, RPO, RO của vendor cụ thể (xem chi tiết chỉ đọc) |
...
- **Bảng vendor:** danh sách có thể sắp xếp hiển thị tên vendor, Vendor ID, trạng thái tài khoản (hoạt động/không hoạt động), ngày đăng nhập cuối, số DO Đã Hoàn Thành, số DO đang chờ (Mới / Đã Gửi)
- **Panel trạng thái sync:** timestamp sync cuối, số vendor đã sync, số vendor bị bỏ qua (không có email), nút "Chạy Sync Ngay"
- Click vào bất kỳ dòng vendor nào điều hướng đến `/admin/vendors/:id` — xem chỉ đọc PO và DO của vendor đó, với nút tải xuống cho DO PDF. **Không có nút mở khoá DO** — admin reset `scheduled_date` trên Odoo trực tiếp (DO-LIVE-001 Blocker 6)
...
- **Chờ Xác Nhận (Waiting)** — PO đang chờ vendor xác nhận
- **Đã Xác Nhận (Confirmed)** — PO đã xác nhận, DO đang ở các trạng thái khác nhau
- **Đã Huỷ (Canceled)** — PO đã bị huỷ
...
- Badge trạng thái PO song ngữ: **Chờ Xác Nhận (Waiting) / Đã Xác Nhận (Confirmed) / Đã Huỷ (Canceled)** (mã màu)
...
- PO ở trạng thái Chờ Xác Nhận (Waiting) hiển thị 2 nút nổi bật: "Xác Nhận Đơn Đặt Hàng" và "Từ Chối"
...
- Banner cảnh báo auto-cancel trên PO Chờ Xác Nhận (Waiting): *"Đơn Đặt Hàng sẽ tự động huỷ sau Ngày Giao Dự Kiến + 7 nếu nhà cung cấp không phản hồi"*
...
- [ ] Cơ chế **live query** Odoo → Portal đã kiểm tra hoạt động ổn (XML-RPC real-time, không cần webhook/polling/batch — theo DO-LIVE-001)
- [ ] Ít nhất một vendor thử nghiệm: nhận lời mời → đặt mật khẩu → đăng nhập → thấy dashboard → xác nhận PO → chỉnh sửa SL Giao trên DO (state Mới) → in DO PDF → giao đến cửa hàng → cửa hàng xác nhận biên nhận trong Odoo → DO Đã Hoàn Thành → nhận email vendor
...
3. **Portal không gọi `button_validate`** — vendor chỉnh sửa DO; Odoo validation do cửa hàng thực hiện. Odoo picking giữ nguyên trạng thái `assigned` sau khi vendor lưu (qty real-time qua PATCH lên `stock.move.line.qty_done`)
4. **Khoá DO chỉ phía portal (UI-only)** — derive từ `scheduled_date.date() <= today (UTC+7)`; **không lưu trạng thái lock trong DB**. Odoo không biết về cơ chế lock này (DO-LIVE-001 Blocker 1)
5. **Không có endpoint mở khoá DO trên Portal** (DO-LIVE-001 Blocker 6) — admin chỉ có thể mở khoá bằng cách cập nhật `scheduled_date` trên Odoo trực tiếp; vendor không có quyền can thiệp vào `scheduled_date`
...
7. **PDF không lưu trên server** — tạo on-the-fly mỗi lần vendor in (DO-LIVE-001 + đã bỏ ký điện tử)
8. **Ghi chú vendor là tuỳ chọn nhưng phải được lưu** — ghi xuống Odoo `stock.picking.note` (vì DO không còn local DB sau DO-LIVE-001)
