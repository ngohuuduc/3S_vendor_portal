...
| Trạng thái DO trên portal | Waiting (chờ ready), Ready (vendor sửa qty_done + note), Locked (scheduled_date ≤ today UTC+7, UI-only), Done (cửa hàng xác nhận), Cancelled — **DO-LIVE-001: không lưu local DB, derive trực tiếp từ Odoo** |
...
| 23:00 Cutoff DO | **DO-LIVE-001:** Khoá UI-only — `scheduled_date <= today (UTC+7)` → Locked. Không có job cutoff cho DO, không push Odoo. Admin mở khoá bằng cách cập nhật `scheduled_date` trực tiếp trên Odoo. RN vẫn dùng cutoff job (bảng `return_notes` còn tồn tại) |
...
- Lúc **23:00 đêm trước ngày giao dự kiến**: DO bị khoá về mặt giao diện (UI-only). Không push Odoo. Vendor không thể chỉnh sửa sau thời điểm này.
