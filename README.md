...
| Thông báo email | Mời dùng, đặt lại mật khẩu, **hủy PO (gửi Buyer + Kho Nhận Hàng)**, xác nhận biên nhận (đơn thuần, không cảnh báo chênh lệch — vendor xem Log Note nếu cần). **Không có email "DO mở khoá"** vì không có chức năng admin unlock trên Portal |
...
- Email tự động gửi Vendor xác nhận biên nhận **đơn thuần** (số PO, mã biên nhận) — **không** kèm cảnh báo chênh lệch. Để xem lịch sử thay đổi `qty_done` (cửa hàng đã sửa hay chưa), vendor xem **Log Note trên DO Form** (mục 8.5).
