...
| Xác nhận ngày giao PO | NCC chọn ngày dự kiến giao hàng trên Portal trước khi xác nhận PO. Portal cập nhật `date_planned` trên `purchase.order`. Ngày vendor chọn không được quá 7 ngày sau `date_planned` của cửa hàng |
...
| Chỉnh sửa DO | Vendor chỉnh số lượng giao (qty <= qty đặt, đúng decimal của UoM: kg → 0.1, units → số nguyên) và ghi chú tổng phiếu giao hàng (header-level). Không chỉnh ngày, không có ghi chú riêng từng dòng SP |
...
- Vendor chọn ngày dự kiến giao (không quá 7 ngày sau `date_planned` của cửa hàng) → nhấp **"Xác Nhận Đơn Đặt Hàng"** → portal cập nhật `date_planned` + gọi `button_confirm` cùng lúc → DO tự động tạo
...
- Portal đọc DO từ Odoo qua XML-RPC, pre-fill qty từ PO, trạng thái **Draft**
- Vendor chỉnh sửa: **số lượng giao** từng dòng (≤ qty đặt, đúng **decimal precision của UoM**: kg → 0.1, units → số nguyên) và **ghi chú tổng phiếu** (một ô text header, không có ghi chú riêng từng dòng SP)
...
**Nút hành động:**
1. Hiển thị ngày mong muốn giao hàng từ cửa hàng (`date_planned`). NCC chọn ngày dự kiến giao (không quá 7 ngày sau `date_planned`). Nhấp **"Xác Nhận Đơn Đặt Hàng"** → portal cập nhật `date_planned` + gọi `button_confirm` cùng lúc
