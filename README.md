...
| 23:00 Cutoff DO | 23:00 đêm trước `scheduled_date` → DO chuyển sang **Đã Gửi (Sent)**. **Chỉ UI lock trên Portal** — derive từ `scheduled_date.date() <= today`. **KHÔNG có job push** — `product_uom_qty` đã được vendor ghi real-time qua PATCH suốt state Mới. **KHÔNG có nightly sync** — Done state cũng derive real-time từ `stock.picking.state`. Admin mở khoá bằng cách reset `scheduled_date` trên Odoo (không có endpoint unlock trên Portal) |
...
| Thông báo email | Mời dùng, đặt lại mật khẩu, **hủy PO (gửi Buyer + Kho Nhận Hàng)**, xác nhận biên nhận (gửi vendor kèm cảnh báo nếu có sai lệch). **Không có email "DO mở khoá"** vì không có chức năng admin unlock trên Portal |
...
- Dữ liệu SL Giao đã được Portal ghi real-time qua PATCH lên `stock.move.product_uom_qty` trước đó. **Không có job push tại 23:00.**
...
|---|---|---|---|
| **Mới (New)** | Draft / `assigned` | PO confirmed, DO được tạo tự động (pre-fill qty từ PO) | Sửa qty + ghi chú tổng phiếu, lưu/in nhiều lần. SL Giao ghi real-time qua PATCH lên `stock.move.product_uom_qty` |
...
1. Chốt SL Giao của vendor (đã được ghi real-time qua PATCH lên `stock.move.product_uom_qty` suốt state Mới)
