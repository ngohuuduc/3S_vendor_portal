...
| Trạng thái DO trên portal | **4 trạng thái** hiển thị label tiếng Việt: **Mới** (= Draft, vendor sửa qty_done + ghi chú tổng phiếu), **Đã Gửi** (= Locked, sau 23:00 cutoff đêm trước ngày giao), **Đã Hoàn Thành** (= Done, cửa hàng xác nhận), **Đã Huỷ** (= Cancelled, khi PO bị huỷ trước khi nhận). **DO-LIVE-001: không lưu local DB, derive trực tiếp từ Odoo** |
...
| Thông báo email | Mời dùng, đặt lại mật khẩu, **hủy PO (gửi Buyer + Kho Nhận Hàng)**, xác nhận biên nhận (gửi vendor kèm cảnh báo nếu có sai lệch), DO được mở khóa (gửi vendor) |
...
|---|---|---|---|
| **Mới** | Draft | PO confirmed, DO được tạo tự động (pre-fill qty từ PO) | Sửa qty + ghi chú tổng phiếu, lưu/in nhiều lần |
| **Đã Gửi** | Locked | 23:00 cutoff đêm trước ngày giao hàng → push `quantity_done` vào `stock.move.line` trên Odoo | Chỉ đọc, in PDF phiên bản cuối |
| **Đã Hoàn Thành** | Done | Cửa hàng đã confirm Receipt trong Odoo (real-time qua webhook/live query) | Chỉ đọc, xem qty thực nhận bên cạnh qty giao, xuất PDF/CSV |
...
| SL Giao | `stock.move.product_uom_qty` | NCC nhập trên Portal — Portal ghi xuống `stock.move.product_uom_qty` qua XML-RPC. **Mặc định = SL Đặt** khi PO confirmed (autofill toàn bộ, NCC chỉ sửa dòng cần sửa). Editable trong state Mới, **freeze giá trị cuối** ở state Đã Gửi/Đã Hoàn Thành (không cho chỉnh nữa). Chỉ chỉnh giảm, không tăng vượt SL đặt. Decimal theo UoM: **kg / lít / mét → 2 chữ số thập phân**, các UoM khác → số nguyên |
| **SL Thực Nhận** *(chỉ state Đã Hoàn Thành)* | `stock.move.quantity_done` | **Mới** — chỉ hiển thị khi cửa hàng đã xác nhận. Là giá trị cửa hàng chốt khi validate picking trên Odoo. Có thể khác SL Giao nếu hàng hư hỏng — vendor đối chiếu được cả 2 cột. (`stock.move.quantity_done` = computed sum của `stock.move.line.qty_done`, tương đương `purchase.order.line.qty_received` ở cấp PO line) |
...
| **Thành Tiền (-V)** | `price_unit × <SL áp dụng> × (1 − discount/100)` (discount là **phần trăm**) | **Mới** — recompute real-time. **Cơ sở tính**: state **Mới** + **Đã Gửi** → dùng SL Giao (`product_uom_qty`); state **Đã Hoàn Thành** → dùng SL Thực Nhận (`quantity_done`) |
| **Tổng Tiền (+V)** | Thành Tiền (-V) + thuế áp dụng | **Mới** — recompute real-time theo cùng cơ sở tính |

**Khu vực tổng cuối phiếu (Summary):** giống PO, **tính lại real-time** — state Mới/Đã Gửi tính theo SL Giao (`product_uom_qty`); state Đã Hoàn Thành tính theo SL Thực Nhận (`quantity_done`):
...
> **Log Note:** xem mục **8.5 Log Note** ở dưới — Log Note **không** áp dụng cho Phiếu Giao Hàng (DO), vì NCC là người chính chỉnh sửa DO. Log Note chỉ áp dụng cho 3 phiếu: Đơn Đặt Hàng (PO), Đơn Trả Hàng (RPO), Phiếu Nhận Hàng (RO).
...
**Trạng thái RO (2 trạng thái — không có Đã Huỷ vì Odoo return picking không có state cancel độc lập):**

| State (kỹ thuật) | Label VN |
|---|---|
| `assigned` / picking active | **Chờ Thu Hồi** |
| `done` | **Đã Hoàn Thành** |
...
- Banner/note trên RO hiển thị: **"Nhà cung cấp vui lòng đến lấy hàng trước hạn chót (dd/mm/yyyy)"** — không dùng text "Phiếu giao nhận bị khoá lúc …" như DO
- In RO PDF giống DO (không cần ký)

#### 8.5 Log Note (Lịch sử thay đổi từ Odoo)

**Phạm vi:** Log Note hiển thị **chỉ trên 3 phiếu**: Đơn Đặt Hàng (PO), Đơn Trả Hàng (RPO), Phiếu Nhận Hàng (RO). **Không** áp dụng cho Phiếu Giao Hàng (DO) — vì NCC là người chính chỉnh sửa DO.
...
- RO picking: `scheduled_date`, `move_ids.product_uom_qty`
