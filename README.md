...
| Trạng thái PO trên portal | **3 trạng thái** song ngữ (VN / EN): **Chờ Xác Nhận (Waiting)** = `sent`, **Đã Xác Nhận (Confirmed)** = `purchase`, **Đã Huỷ (Canceled)** = `cancel`. Draft (`draft`) không hiển thị. Tự động huỷ sau 7 ngày dương lịch kể từ Expected Arrival nếu vendor chưa xác nhận hoặc từ chối |
...
| Tóm tắt dashboard Vendor | Số lượng PO theo trạng thái (Chờ Xác Nhận / Đã Xác Nhận / Đã Huỷ) hiển thị phía trên danh sách PO |
...
- **Chờ Xác Nhận (Waiting)** — RFQ đã được gửi cho vendor và đang chờ xác nhận. Vendor có thể xác nhận hoặc từ chối.
- **Đã Xác Nhận (Confirmed)** — PO đã được duyệt, DO đã được tạo, Vendor có thể in PDF
- **Đã Huỷ (Canceled)** — PO đã bị hủy (vendor từ chối, hoặc cửa hàng hủy). Chỉ đọc.

RFQ ở trạng thái Draft không được hiển thị. Vendor có thể xem dữ liệu PO trong **3 tháng** gần nhất.

**Xác nhận đơn đặt hàng:**
- Trang chi tiết PO ở trạng thái Chờ Xác Nhận (Waiting) hiển thị ngày mong muốn giao hàng từ cửa hàng (`date_planned`)
...
- Nút "Từ Chối" hiển thị cùng trang chi tiết PO ở trạng thái Chờ Xác Nhận (Waiting)
...
**Tự động hủy:**
- Nếu vendor không xác nhận hoặc từ chối PO ở trạng thái Chờ Xác Nhận (Waiting) trong vòng **7 ngày dương lịch kể từ Expected Arrival date** (`date_planned` trên `purchase.order`), **Portal tự động hủy trong Odoo** (Portal cron job gọi `button_cancel` qua XML-RPC) và **Portal tự gửi email** đến Vendor + Buyer + Kho Nhận Hàng.
...
- Một scheduled job kiểm tra hằng ngày các PO Chờ Xác Nhận (Waiting) quá hạn
...
| **Đã Xác Nhận (Confirmed)** | `purchase` | Vendor xác nhận PO | Xem DO, xuất dữ liệu |
| **Đã Huỷ (Canceled)** | `cancel` | Vendor từ chối hoặc cửa hàng hủy | Chỉ đọc |
...
**Banner cảnh báo tự động huỷ (chỉ hiển thị khi PO ở trạng thái Chờ Xác Nhận (Waiting)):**
